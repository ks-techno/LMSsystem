...
class Issue_m extends MY_Model {
	...
	public function get_issue_count_group_by_books($array = NULL, $status = FALSE) {
		$this->db->select('bookID,COUNT(issueID) as total');
		$this->db->from('issue'); 
	...
	public function get_issue_by_array($array = NULL, $status = FALSE) {
		$this->db->select('*');
		$this->db->from('issue');
	...
	public function get_issue_lastID_by_add() {
		$this->db->select('*')->from('issue')->order_by('issueID  desc')->limit(1, 0);
		$query = $this->db->get();
		return $query->row()->issueID+1;
	}
}