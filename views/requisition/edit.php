                ...
                <form role="form" method="post" enctype="multipart/form-data" id="requisitionDataForm"> 
                    ...
                    ?>
                        ...
                        <div >
                            <?php
                                ...
                                echo form_dropdown("usertypeID", $userArray, set_value("usertypeID",$requisition->usertypeID), "id='usertypeID' class='form-control select2'");
                            ?>
                        </div>
                    ...
                    ?>
                        ...
                        <div class="">
                            <?php
                                ...
                                echo form_dropdown("userID", $userArray, set_value("userID",$requisition->userID), "id='userID' class='form-control select2'");
                            ?>
                        </div>
                        <span class="col-sm-4 control-label">
                    ...
                    <div class="requisitiondateDiv form-group <?=form_error('requisitiondate') ? 'has-error' : '' ?>" >
                        <label for="requisitiondate">
                            ...
                            <input type="text" class="form-control" id="requisitiondate" name="requisitiondate" value="<?=set_value('requisitiondate', date('d-m-Y', strtotime($requisition->requisitiondate)))?>" >
                        <span class="text-red">
                    ...
                    <div class="form-group <?=form_error('requisitiondescription') ? 'has-error' : '' ?>" >
                        ...
                        <textarea class="form-control" style="resize:none;" id="requisitiondescription" name="requisitiondescription"><?=set_value('requisitiondescription', $requisition->requisitiondescription)?></textarea>
                        <span class="text-red">
                            <?php echo form_error('requisitiondescription'); ?>
                        </span>
                    </div>

                    <input id="addPurchaseButton" type="button" class="btn btn-success" value="<?=$this->lang->line("update_requisition")?>" >
                </form>
            </div>
        </div>
    </div>
        ...
        <div class="box">
            <div class="box-header">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="active"><?=$this->lang->line('menu_edit')?> <?=$this->lang->line('menu_requisition')?></li>
                </ol>
            </div><!-- /.box-header -->
            <div class="box-body">
                    ...
                    <table class="table table-bordered product-style" style="font-size: 16px;">
                        ...
                        <tbody id="productList">
                            <?php 
                                $totalQuantity = 0;
                                $totalSubtotal = 0;
                                if(customCompute($requisitionitems)) {
                                    $i=1; 
                                    foreach ($requisitionitems as $requisitionitem) {
                                        $randID = rand(1, 9999999999);
                                        $totalQuantity += $requisitionitem->requisitionquantity;
                                        $subtotal = ($requisitionitem->requisitionunitprice * $requisitionitem->requisitionquantity);
                                        $totalSubtotal += $subtotal; 
                                        echo '<tr id="tr_'.$randID.'" purchaseproductid="'.$requisitionitem->productID.'">';
                                            echo '<td>';
                                                echo $i;
                                            echo '</td>';

                                            echo '<td>';
                                                echo isset($products[$requisitionitem->productID]) ? $products[$requisitionitem->productID] : '';
                                            echo '</td>';

                                             

                                            echo '<td>';
                                                echo '<input type="text" class="form-control change-productquantity" id="productquantity_'.$randID.'" data-productquantity-id="'.$randID.'" value="'.$requisitionitem->requisitionquantity.'">';
                                            echo '</td>';
...
<script type="text/javascript">
    ...
    $(document).on('click', '#addPurchaseButton', function() {
        ...
        var productitem = $('tr[id^=tr_]').map(function(){
            return { productID : $(this).attr('purchaseproductid'), unitprice: 1, quantity : $(this).children().eq(2).children().val() };
        }).get();
        ...
        if(error === 0) {
            ...
            formData.append("editID", <?=$requisitionID?>);
