        ...
        <form class="form-horizontal" role="form" method="post" id="templateDataForm">
            <div class="row">
                <div class="col-sm-12" style="margin-bottom: 10px;">
                    <div class="classesDiv form-group <?=form_error('salary_grades') ? 'has-error' : '' ?>" >
                        ...
                        <div class="col-sm-4">
                            <?php
                                ...
                                echo form_dropdown("salary_grades", $classesArray, set_value("salary_grades"), "id='salary_grades' class='form-control select2'");
                            ?>
                        </div>    
                    ...
                    ?>
                        ...
                        <div class="col-sm-4">
                            <textarea class="form-control" rows="5" id="overtime_rate" name="overtime_rate"><?=set_value('overtime_rate')?></textarea>
                        ...
                        <div class="box-body">
                            <div class="row">
                                <div class="col-sm-12" id="allowances">
                                    ...
                                    ?>
                                      <!-- start -->
                                    <div id="allowances_main_<?php echo $sr;?>" class='form-group allowancesfield'>
                                        <div class="col-sm-5">
                                            <input type="hidden" class="form-control" id="allowanceslabel<?php echo $sr;?>" name="allowanceslabel<?php echo $sr;?>" value="<?php echo $ft->feetypesID?>"  >
                                            <input type="text" class="form-control" id="allowances-label<?php echo $sr;?>" name="allowances-label<?php echo $sr;?>" value="<?php echo $ft->feetypes;?>" readonly  >
                                        </div>

                                        <div class="col-sm-5">
                                            <input type="text" class="form-control allowancesamount" id="allowancesamount<?php echo $sr;?>" name="allowancesamount<?php echo $sr;?>" required  >
                                        </div>

                                        <div class="col-sm-2"><button type="button" class="btn btn-danger btn-xs salary-btn salary-btn-allowances-remove" id="<?php echo $sr;?>" onclick="removeAllowances(this)"><i class="fa fa-trash"></i></button></div>

                                        

                                        <span class="col-sm-12 errorpointallowances" id="allowanceserror1">
                                            <?php echo form_error('amount'.$sr); ?>
                                        </span>
                                    </div>
...
<script type="text/javascript">
    ...
    function removeAllowances(clickedElement) {
        ...
        for(i=1; i<=totalOption; i++) {
            if(i != totalOption) {
                $('#allowances').append(formHtmlData(type, i, label[i], amount[i],label_hidden[i], add='', remove=1));
            } else if(i == 1) {
                $('#allowances').append(formHtmlData(type, i, label[i], amount[i],label_hidden[i], add='', remove=''));
            } else {
                $('#allowances').append(formHtmlData(type, i, label[i], amount[i],label_hidden[i], add='', remove=1));
            }
        }
    ...
    function formHtmlData(type, id, label, amount,label_hidden='', add, remove) {
        if(type == 'allowances') {
        ...
        var html = '<div class="form-group '+type+'field" ><div class="col-sm-5"><input type="hidden" class="form-control" id="'+type+'label'+id+'" name="'+type+'label'+id+'" value="'+label+'" placeholder="'+langLabel+'"><input type="text" class="form-control" id="'+type+'-label'+id+'" name="'+type+'-label'+id+'" value="'+label_hidden+'" readonly placeholder="'+langLabel+'"></div><div class="col-sm-5"><input type="text" class="form-control '+type+'amount" id="'+type+'amount'+id+'" name="'+type+'amount'+id+'" value="'+amount+'" placeholder="'+langValue+'"></div><div class="col-sm-2" >'+button+'</div><span class="col-sm-12 errorpoint'+type+'" id="'+type+'error'+id+'"><?php echo form_error('amount1'); ?></span></div>';
    ...
    $('#addSalaryTemplate').click(function() {
        ...
        if(salary_grades == '') {
            $('#salary_grades_error').html('The Course name field is required.');
        ...
        if(basic_salary == '') {
            $('#basic_salary_error').html('The Year field is required.');
        ...
        if(overtime_rate == '') {
            $('#overtime_rate_error').html('The Description field is required.');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(response.status == 'success') {
                        window.location = "<?=base_url("courses/index")?>";
                    } else {
