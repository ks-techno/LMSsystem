<div class="row">
                ...
                <form role="form" method="post" enctype="multipart/form-data" id="productPurchaseDataForm"> 
                           ...
                           // $usertypeArray = array(0 => $this->lang->line("productsale_select_role"));
                            if(customCompute($usertypes)) {
                                ...
                                foreach ($usertypes as $usertype) {
                                    if ($requisition->usertypeID==$usertype->usertypeID) {
                    ...
                    <div class="productsalereferencenoDiv form-group <?=form_error('productsalereferenceno') ? 'has-error' : '' ?>" >
                        <label for="productsalereferenceno">
                            ...
                            <input type="text" class="form-control" id="productsalereferenceno" readonly name="productsalereferenceno" value="<?=set_value('productsalereferenceno',$requisition->refrence_no)?>" >
                        <span class="text-red">
                    ...
                    <div class="hide productsalepaymentstatusDiv form-group <?=form_error('productsalepaymentstatusID') ? 'has-error' : '' ?>" >
                        ...
                        <?php
                            ...
                            echo form_dropdown("productsalepaymentstatusID", $productwarehouseArray, set_value("productsalepaymentstatusID" ,1), "id='productsalepaymentstatusID' class='form-control select2'");
                        ?>
                        <span class="text-red">
    ...
    <div class="col-sm-9">
        <div class="box">
            ...
            <div class="box-body">
                <form class="hide" role="form" method="post" enctype="multipart/form-data">
                    <div class="row">
                        <div class="col-sm-6">
                    ...
                    <table class="table table-bordered product-style" style="font-size: 16px;">
                        <thead>
                            <tr>
                                ...
                                <th class="col-sm-2">Available <?=$this->lang->line('productsale_quantity')?></th>
                        ...
                        <tbody id="productList"> 
                            <?php 
                                $productpurchasequintityobj =   $productpurchasequintity;
                                $productsalequintityobj     =   $productsalequintity;
                                ...
                                if(customCompute($productsaleitems)) {
                                    foreach ($productsaleitems as $productsaleitem) {
                                        if(isset($totalEditQuantity[$productsaleitem->productID])) {
                                            $totalEditQuantity[$productsaleitem->productID] += $productsaleitem->requisitionquantity;
                                        } else {
                                            $totalEditQuantity[$productsaleitem->productID] = $productsaleitem->requisitionquantity;
                                        }
                                    }
                                ...
                                if(customCompute($productsaleitems)) {
                                    ...
                                    foreach ($productsaleitems as $productsaleitem) {
                                        ...
                                        $totalQuantity += $productsaleitem->requisitionquantity;
                                        $subtotal = ($productsaleitem->requisitionunitprice * $productsaleitem->requisitionquantity);
                                        ...
                                        echo '<tr id="tr_'.$randID.'" saleproductid="'.$productsaleitem->productID.'">';
                                            ...
                                            echo '<td>';
                                            //var_dump($productsaleitem);
                                            echo isset($products[$productsaleitem->productID]) ? $products[$productsaleitem->productID] : '';
                                            echo '</td>';

                                            echo '<td>';
                                            echo '<input type="text" class="form-control change-productprice" id="productunitprice_'.$randID.'" data-productprice-id="'.$randID.'" value="'.$productsaleitem->requisitionunitprice.'">'.($totalProductStockQuantity[$productsaleitem->productID]);
                                            echo '</td>';

                                            echo '<td>';
                                            echo '<input type="text"   class="form-control change-productquantity" id="productquantity_'.$randID.'" data-productquantity-id="'.$randID.'" value="'.$productsaleitem->requisitionquantity.'" max="'.($totalProductStockQuantity[$productsaleitem->productID]  ).'">';
                                            echo '</td>';
                                            echo '<td>';
                                            echo '<a style="margin-top:3px" href="#" class="hide  btn btn-danger btn-sm deleteBtn" id="productaction_'.$randID.'" data-productaction-id="'.$randID.'"><i class="fa fa-trash-o"></i></a>';
                                            echo '</td>';
                                        echo '</tr>';
                                        $i++; 
                                    }
    ...
    <div class="error-content">
        <h3><i class="fa fa-warning text-yellow"></i> Requisition Already Proceeded.</h3>
        <p>
            We could not find the page you were looking for.
            Meanwhile, you may <a href="<?php echo base_url('/');?>dashboard/index">return to dashboard</a> or try using the search form.
        </p>
    </div><!-- /.error-content -->
</div>
        </div>
...
<script type="text/javascript">
    ...
    $(document).on('click', '#addPurchaseButton', function() {
        ...
        if(error === 0) {
            ...
            formData.append("requisitionID", <?php echo $requisitionID;?>);
    ...
    function errrorLoader(response) {
        if(response.status) {
            window.location = "<?=base_url("productsale/print_preview/")?>"+response.productsaleID;
        } else {
