    ...
    <div class="col-sm-7">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title"><i class="fa icon-invoice"></i> <?='Add Other Charges'?></h3>
            </div><!-- /.box-header -->
            <div class="box-body"> 
                <form role="form" method="post" enctype="multipart/form-data" id="invoiceDataForm"> 
                    ...
                    <!-- <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            ...
                            <label class="radio-inline">
                              <input type="radio" name="feess_type" id="feess_type" value="Lump Sum">Lump Sum
                            </label> -->
                    ...
                    <div class="chargesDiv form-group <?=form_error('feesType') ? 'has-error' : '' ?>" >
                        <label for="Fees type">
                            <?=$this->lang->line("invoice_fee_types")?> <span class="text-red">*</span>
                        </label>
                        <?php
                            $statusArray = array(
                                0 => $this->lang->line("Select_fee_types"),
                                20 => $this->lang->line("late_payment_fine"),
                                21 => $this->lang->line("hostel_dues"),
                                22 => $this->lang->line("short_attendance"),
                                23 => $this->lang->line("discipline_fine"),
                                24 => $this->lang->line("supply_charges"),
                                25 => $this->lang->line("others"),
                                26 => $this->lang->line("result"),
                            );
                    ...
                    <div class="ammountDiv form-group" >
                        <label for="ammount">
                            <?=$this->lang->line("charges_ammount")?> <span class="text-red">*</span>
                        </label>
...
<script type="text/javascript">
    ...
    $(document).on('click', '#addInvoiceButton', function() {
        ...
        var field = {
            ...
            'charges_ammount'     : $('#charges_ammount').val(),
        ...
        if(field['charges_ammount'] === '') {
            $('.ammountDiv').addClass('has-error');
            error++;
        } else {
            $('.ammountDiv').removeClass('has-error');
        }
        ...
        // if(field['feess_type'] != 0) {
            if(field['feess_type'] === '0') {
                $('.chargesDiv').addClass('has-error');
                error++;
            } else {
                $('.chargesDiv').removeClass('has-error');
            }
        // }
    ...
    function ajaxCall(passData) {
        $.ajax({
            type: 'POST',
            url: "<?=base_url('other_charges/saveinvoice')?>",
            data: passData,
            async: true,
            dataType: "html",
            success: function(data) {
    ...
    function errrorLoader(response) {
        if(response.status) {
            window.location = "<?=base_url("other_charges/index")?>";
        } else {
