...
<div class="box">
    <div class="box-header">
        <h3 class="box-title"><i class="fa icon-invoice"></i> <?='Other Charges'?></h3>
    ...
    <div class="box-body">
        <div class="row">
            <div class="col-sm-12">
                        ...
                        <?php if(permissionChecker('invoice_add')) { 
                                    ?>
                                     <h5 class="page-header">
                                        <a href="<?php echo base_url('other_charges/add') ?>">
                ...
                <div class="col-sm-9">
                    <!-- <div class="col-sm-3">
                        <label>Upload CPV</label>
                        <input type="file" name="cpv">
                    </div>
                     <div class="col-sm-3">
                        <label>Upload BPV</label>
                        <input type="file" name="bpv">
                    </div> -->
                    <form method="post">
                    <div class="col-sm-4 col-md-offset-5">
                        <input type="text" class="form-control" name="searchInvoice">
                    </div>
                    <div class="col-sm-2">
                        ...
                        <tbody>
                            <?php if(customCompute($maininvoices)) {$i = 1; foreach($maininvoices as $maininvoice) { ?>
                                <tr>
                                    ...
                                    <?php
                                     if(permissionChecker('other_charges_view') || permissionChecker('other_charges_edit') || permissionChecker('other_charges_delete')) { 
                                        ?>
                                    <td data-title="<?=$this->lang->line('action')?>">
                                        <?php
                                            if ($maininvoice->maininvoice_status == 1){
                                                echo btn_view('other_charges/view/'.$maininvoice->maininvoiceID, $this->lang->line('view'));
                                            } 
                                             ?>
                                        <?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) { ?>
                                            <?php if($maininvoice->maininvoicestatus != 1 && $maininvoice->maininvoicestatus != 2 && $maininvoice->maininvoice_status == 1) { echo btn_edit('other_charges/edit/'.$maininvoice->maininvoiceID, $this->lang->line('edit')); } ?>
                                            <?php if($maininvoice->maininvoicestatus != 1 && $maininvoice->maininvoicestatus != 2 && $maininvoice->maininvoice_status == 1) { echo btn_delete('other_charges/delete/'.$maininvoice->maininvoiceID, $this->lang->line('delete')); } ?>
                                        <?php } ?>
                                        <?php if(permissionChecker('other_charges_view')) { if($maininvoice->maininvoicestatus != 2 && $maininvoice->maininvoice_status == 1) { echo btn_invoice('other_charges/payment/'.$maininvoice->maininvoiceID, $this->lang->line('payment')); }} ?>
                                        <?php 
                                            if(permissionChecker('other_charges_view')) { 
