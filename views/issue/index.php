                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td data-title="<?=$this->lang->line('issue_book')?>">
                                                <?php echo $issue->accession_number; ?>
                                            </td>
                                                ...
                                                <?php
                                                    if(permissionChecker('issue_add') && permissionChecker('issue_edit')   ) {
                                                        if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
                                                             ...
                                                             if ($issue->usertypeID==3) {
                                                              
                                                                echo '<a onclick="return_book(\''.$issue->issueID.'\')" href="javascript:;" class="btn btn-xs btn-danger mrg"  data-toggle="modal" rel="tooltip">';
...
<?php if(customCompute($issues)) { ?>
    <!-- For invoice -->
    <form class="form-horizontal" role="form" action="<?=base_url('issue/add_invoice');?>" method="post">
        <div class="modal fade" id="invoice">
          <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body" id="invoice_modal_body"></div>
                    
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" style="margin-bottom:0px;" data-dismiss="modal"><?=$this->lang->line('close')?></button>
                    <input type="submit" id="add_invoice" class="btn btn-success" value="<?=$this->lang->line("issue_add_fine")?> and Return" />
                </div>
            </div>
          </div>
        </div>
...
$("#due_date").datepicker();
      ...
      function return_book(issueID) {

         
           $.ajax({
                          type: 'POST',
                          url: "<?=base_url('issue/get_issue_data')?>",
                          data: {'issueID' : issueID},
                          dataType: "html",
                          success: function(data) {
                             $('#invoice_modal_body').html(data);
                             $('#invoice').modal('show');
                             $("#due_date").datepicker();
                          }
                      });
      }
      function display_fine_detail( ) {
         var finestatus   =  $('input[name="include_fine"]:checked').val();
          
         if (finestatus==1) {
          $('#fine_detail').show('slow');
          $("input#amount").prop('required',true);
          $("input#due_date").prop('required',true);
         }else{

          
          $("input#amount").prop('required',false);
          $("input#due_date").prop('required',false);
          $('#fine_detail').hide('slow');
         }
      }
    </script>
<?php } ?>