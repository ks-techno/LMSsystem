...
*/
	...
	public function rules($reportfor= 'blood') {		
		...
		if($reportfor == "active") {
			$rules[] = array(
				'field' => 'active',
				'label' => $this->lang->line("studentreport_status"),
				'rules' => 'trim|required|xss_clean|callback_unique_data'
			);
		} elseif($reportfor == "blood") {
			$rules[] = array(
				'field' => 'blood',
	...
	private function getArray(&$queryArray, $post) {
		...
		if(isset($post['reportfor']) && $post['reportfor'] == 'active') {
			$queryArray['active'] = $post['active'];
			$this->data['reportTitle'] = $post['active'];
		}
	...
	public function pdf() {
		if(permissionChecker('studentreport')) {
			...
			$active      = htmlentities(escapeString($this->uri->segment(12)));
			...
			if((string) $reportfor && ((int) $active || $active >= 0) && ((int) $blood || $blood >= 0) && ((int) $country || $country >= 0) && ((int) $transport || $transport >= 0) && ((int) $gender || $gender >= 0) && ((int) $birthdaydate || $birthdaydate >= 0) && ((int) $classesID || $classesID >= 0) && ((int) $sectionID || $sectionID == NULL || $sectionID >= 0)) {
	...
	private function xmlData()
	{
		...
		$reportforArray = array('active','blood','country','gender','transport','hostel','birthday');
