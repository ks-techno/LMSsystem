		...
		if(permissionChecker('productsalereport')) {
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
                    ...
                    $this->data['productsales'] = $productsales;

                    $products = pluck($this->product_m->get_product(), 'productname', 'productID');
                    $allsale   =   array();
                    foreach ($productsales as $ps) {

                    $new_productsales   =   array();
                       $pros = $this->productsaleitem_m->get_order_by_productsaleitem(array('productsaleID' => $ps->productsaleID));
                       $all_p   =   '';
                       foreach ($pros as $p) {
                         
                          $all_p   .=   $products[$p->productID].':'.$p->productsalequantity.", \r \n ";
                       }
                        $new_productsales['productsalereferenceno']   =   
                                    $ps->productsalereferenceno;
                        $new_productsales['productsalerefund']   =   
                                    $ps->productsalerefund;
                        $new_productsales['productsalecustomertype']   =   
                                    isset($usertypes[$ps->productsalecustomertypeID]) ? $usertypes[$ps->productsalecustomertypeID] : '';
                        $name = '';
                            if(isset($users[$ps->productsalecustomertypeID][$ps->productsalecustomerID])) {
                                $name = isset($users[$ps->productsalecustomertypeID][$ps->productsalecustomerID]->name) ? $users[$ps->productsalecustomertypeID][$ps->productsalecustomerID]->name : $users[$ps->productsalecustomertypeID][$ps->productsalecustomerID]->srname;
                            }
                        $new_productsales['productsalecustomerName']   =   
                                    $name;
                        $new_productsales['productsaledate']   =   
                                    $ps->productsaledate;
                        $new_productsales['allproducts']   =   
                                    $all_p; 
                        $allsale[]  =   $new_productsales;

                    }
                    
                    $this->data['productsales'] = $allsale;
	...
	private function generateXML($arrays) {
		...
		if(customCompute($productsales)) {
            ...
            foreach($productsales as $productsale) {
                ...
                $bodys[$i][] = $productsale['allproducts']; 
                $i++;
            }
