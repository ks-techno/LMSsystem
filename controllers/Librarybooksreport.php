...
class Librarybooksreport extends Admin_Controller{
        ...
        if(permissionChecker('librarybooksreport')) {
			if($_POST) {
			    ...
			    if ($this->form_validation->run() == FALSE) {
					...
					if(!empty($this->data['classesID'])) {
						$postArray['classesID'] = $this->data['classesID'];
					}

					$issuedbooks 	= 	$this->issue_m->get_issue_count_group_by_books();
					$issued 		=	[];
					foreach ($issuedbooks as $ib) {
						 $issued[$ib->bookID] 	=	$ib->total;
					}
					$notreturned 	= 	$this->issue_m->get_issue_count_group_by_books(array('return_date IS NOT NULL'=>NULL));
					$nonreturn 		=	[];
					foreach ($notreturned as $nr) {
						 $nonreturn[$nr->bookID] 	=	$nr->total;
					}
					...
					if(customCompute($books)) {
						foreach($books as $book) {
							...
							if(isset($issued[$book->bookID])){
							$op1 =	$issued[$book->bookID];
							}else{
							$op1 =	'-';
							}
							if(isset($nonreturn[$book->bookID])){
								$op2 =	$nonreturn[$book->bookID];
							}else{
								$op2 =	'-';
							}
							...
							$bookArray[$i]['due_quantity'] 	= $book->due_quantity;
							$bookArray[$i]['quantity'] 		= $book->quantity;
	                        ...
	                        $bookArray[$i]['totalissued'] 	= $op1;
	                        $bookArray[$i]['notreturned'] 	= $op2;
						}
					}
	...
	private function generateXML($data) {
		...
		if(customCompute($books)) {
            ...
            $headers['due_quantity'] = 'Issued';
            $headers['totalissued'] = 'Total Issued';
            $headers['notreturned'] = 'Not Returned';
            ...
            foreach($books as $book) { 
                ...
                $body[$i][] = $book['rackNo'];
                $body[$i][] = $book['status'];
                $body[$i][] = $book['quantity'];
                $body[$i][] = $book['due_quantity'];
                $body[$i][] = $book['totalissued'];
                $body[$i][] = $book['notreturned'];
            }
