...
class Asset extends Admin_Controller {
    ...
    */
    ...
    public function index() {
        ...
        $this->data['assets'] = $this->asset_m->get_asset_with_category_and_location($array);
    ...
    public function add() {
        ...
        if($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $get_single = $this->asset_category_m->get_single_asset_category(['asset_categoryID' => $this->input->post("asset_categoryID")]);
                $category_abbr = strtoupper($get_single->category_abbr);
                // echo '<pre>';
                // print_r($get_single->category_abbr);
                // echo '</pre>';
                // echo '<br>';
                $settings = $this->setting_m->get_setting_where('sname');
                // echo '<br>';
                $ref_last = $this->asset_m->get_asset_ref($this->input->post("asset_categoryID"));
                if ($ref_last !== false) {
                    // preg_replace("/[^0-9]{1,4}/", '', $ref_last);
                    $num = preg_replace("/[^0-9]/", '', $ref_last);
                    $num = $num + 1;
                    // print_r(preg_replace("/[^0-9]/", '', $ref_last));
                    // $ref_last = 
                }else{
                    $num = 001;
                }
                // echo '<br>';
                // var_dump($this->asset_m->get_asset_ref($this->input->post("asset_categoryID")));
                // echo '<br>';
                $sname      = $settings->value;
                $sname_l    = strlen($sname);
                if ($sname_l > 4) {
                    $first4 = substr($sname, 0, 4);
                    // echo '<br>';
                    $first4 = strtoupper($first4);
                    // echo '<br>';
                    $ref = $first4.'-'.$category_abbr.'-'.$num;
                    // echo '<br>Hello';
                    // echo $theRest = substr($sname, 4);
                }else{
                     $sname = strtoupper($sname);
                     $ref = $sname.'-'.$category_abbr.'-'.$num;
                    // echo 'success';
                }
                // echo $get_single->category_abbr;
                // echo '<pre>';
                // print_r($settings);
                // echo '<pre>';
                // exit();
    ...
    public function ledger() {
        ...
        if((int)$id) {
                $this->data['asset']        = $this->asset_m->get_single_asset_with_category_and_location(array('asset.assetID' => $id));
            if(customCompute($this->data['asset'])) {
                $this->data['allinouts']    = $this->assets_product_inout_m->get_order_by_assets_product_inout(array('productID' => $id));
                $this->data["subview"] = "asset/ledger";
                $this->load->view('_layout_main', $this->data);
            } else {
                $this->data["subview"] = "error";
                $this->load->view('_layout_main');
    ...
    public function ledger_print_preview() {
        if(permissionChecker('asset_view')) {
            ...
            if((int)$assetID) {
                ...
                if(customCompute($this->data['asset'])) {
                    $this->data['allinouts']    = $this->assets_product_inout_m->get_order_by_assets_product_inout(array('productID' => $assetID));
                   // $this->load->view('asset/ledger_print_preview', $this->data);
                    $this->reportPDF('assetmodule.css',$this->data, 'asset/ledger_print_preview');
                } else {
                    $this->data["subview"] = "error";
                    $this->load->view('_layout_main');
                }
    ...
    public function getassetlist() {
        $asset_categoryID = $this->input->post('asset_categoryID');
        if((int)$asset_categoryID) {
            echo "<option value='0'>Please Select</option>";
            $allcategories = $this->asset_m->get_order_by_asset(array('asset_categoryID' => $asset_categoryID));
