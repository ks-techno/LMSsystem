...
class Onlineadmissions extends Admin_Controller {
/*
| -----------------------------------------------------
...
*/
	function __construct () {
		...
		$this->load->model("onlineadmissions_m");
		...
		$this->lang->load('onlineadmissions', $language);
	}
    ...
    public function add_bank_rules(){
        $rules = array(
             array(
				'field' => 'bank_accounts',
				'label' => $this->lang->line("onlineadmission_bank"),
				'rules' => 'trim|required|xss_clean'
			)
        );
        return $rules;
    ...
    public function index() {
        ...
        if((int)$id) {
            ...
            $this->data['onlineadmissions'] = $this->onlineadmissions_m->get_where_in_onlineadmission(array(0, 2), 'status', array('classesID' => $id));
        } else {
            // $this->data['onlineadmissions'] = $this->onlineadmissions_m->get_order_by_onlineadmission(['status' => 'In process']);
            $this->data['onlineadmissions'] = $this->onlineadmissions_m->get_onlineadmission_with_status();
            // echo '<pre>';
            // print_r($this->data['onlineadmissions']);
            // echo '</pre>';
            // exit();
	        $this->data['classesID'] = 0;
        }
        $this->data["subview"] = "onlineadmissions/index";
        $this->load->view('_layout_main', $this->data);
    }

    public function view() {
        if(permissionChecker('onlineadmissions')) {
            $id = htmlentities(escapeString($this->uri->segment(3)));
            if((int)$id) {
                $this->data['admissioninfo'] = $this->onlineadmissions_m->get_invoice_data(array('id' => $id));
                // $this->data['admissioninfo'] = $this->onlineadmissions_m->get_where_in_onlineadmission(array(0, 2), 'status', array('onlineadmissionID' => $id));
                $this->admissionPDF('admissionsmodule.css',$this->data, 'onlineadmissions/print_preview');
                ...
                if(customCompute($this->data['admissioninfo'])) {
                    ...
                    if($_POST) {
                        ...
                        if ($this->form_validation->run() == FALSE) {
                            ...
                            $this->onlineadmissions_m->update_onlineadmission(array('status' => 1, 'studentID' => $studentID), $id);
    ...
    public function download_documentd() {
        ...
        if((int)$id && (int)$classesID) {
            if((permissionChecker('onlineadmission'))) {
            $document = $this->onlineadmissions_m->get_single_onlineadmission(array('onlineadmissionID' => $id));
    ...
    public function decline() {
        if(permissionChecker('onlineadmissions')) {
            ...
            // $url = htmlentities(escapeString($this->uri->segment(4)));
            if((int)$id) {
                $onlineadmission = $this->onlineadmissions_m->get_where_in_onlineadmission(array('In process', 'Pending', 'Confirmed'), 'status', array('id' => $id));
                // var_dump($onlineadmission);
                // exit();
                if(customCompute($onlineadmission)) {
                    $this->onlineadmissions_m->update_onlineadmission(array('status' => 'Rejected'), $id);
                    $this->session->set_flashdata('success', $this->lang->line('menu_success'));
                    redirect(base_url("onlineadmissions/index/"));
                }  else {
                    $this->data["subview"] = "error";
                    $this->load->view('_layout_main', $this->data);
                }
    ...
    public function waiting() {
        if(permissionChecker('onlineadmission')) {
			...
			$url = htmlentities(escapeString($this->uri->segment(4)));
			if((int)$id && (int)$url) {
                $onlineadmission = $this->onlineadmissions_m->get_where_in_onlineadmission(array(0, 2), 'status', array('onlineadmissionID' => $id));
                if(customCompute($onlineadmission)) {
                    $this->onlineadmissions_m->update_onlineadmission(array('status' => 2), $id);
                    $this->session->set_flashdata('success', $this->lang->line('menu_success'));
                    redirect(base_url("onlineadmission/index/".$url));
                }  else {
                    $this->data["subview"] = "error";
    ...
    public function bankadd(){
        if(permissionChecker('onlineadmissions')) {
            if($_POST){
                    if(permissionChecker('onlineadmissions')){
                        ...
                        $this->form_validation->set_rules($rules);
                         if ($this->form_validation->run() == FALSE) {
                            ...
                            if((int)$id){
                                $results = $this->onlineadmissions_m->change_paid_status($id, $admission_fee, $bank_accounts);
                                $this->session->set_flashdata('success', $this->lang->line('menu_success'));
                                redirect(base_url("onlineadmissions/index/"));
                            }else{
                                $this->session->set_flashdata('success', $this->lang->line('onlineadmission_data_not_found'));
                                redirect(base_url("onlineadmissions/index/"));
                                // $retArray['message'] = $this->lang->line('onlineadmission_data_not_found');
                                // echo json_encode($retArray);
                                // exit;
                    ...
                    $this->session->set_flashdata('success', $this->lang->line('onlineadmission_permission'));
                    redirect(base_url("onlineadmissions/index/"));
                    // $retArray['message'] = $this->lang->line('onlineadmission_permission');
                    // echo json_encode($retArray);
                    // exit;
            ...
            $this->data['bank_accounts'] = $this->onlineadmissions_m->get_bank_accounts();
            $getsingle = $this->onlineadmissions_m->get_single_onlineadmission(['id' => $id]);
            $this->data['admission_fee'] = ['admission_fee' => $getsingle->admission_fee];
            // $this->data['schoolyears'] = $this->schoolyear_m->get_schoolyear();
            $this->data["subview"] = "onlineadmissions/bankadd";
            $this->load->view('_layout_main', $this->data);
            // exit();
        }else{
            $this->data["subview"] = "error";
	...
	public function photoupload() {
		...
		if((int)$id) {
			$onlineadmission = $this->onlineadmissions_m->get_single_onlineadmission(array('onlineadmissionID'=>$id));
		}
