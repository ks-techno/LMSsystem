    ...
    public function index() {
        ...
        $this->data['headerassets'] = array(
            ...
            if ($_GET) {
                ...
                if ($status != 0) {
                    $array["asset_assignment.status"]         = $status;
        ...
        $this->data['asset_assignments'] = $this->asset_assignment_m->get_asset_assignment_with_userypeID($array);
    ...
    public function add() {
        ...
        if($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $assignmentID    =    $this->asset_assignment_m->insert_asset_assignment($array);

                $single_asset = $this->asset_m->get_single_asset(['assetID' => $this->input->post("assetID")]);
                $asset_quantity = $single_asset->quantity;
                if($this->input->post("status") == 1){
                    $asset_quantity = $asset_quantity - $this->input->post("assigned_quantity");
                    $array1 =    array(
                                                'type'              => 'stockout', 
                                                'productID'         => $single_asset->assetID, 
                                                'quantity'          => $this->input->post("assigned_quantity"), 
                                                'price'             => $single_asset->average_price, 
                                                'createuserID'      => $this->session->userdata('loginuserID'), 
                                                'createusertypeID'  => $this->session->userdata('usertypeID'), 
                                                'date'              => date('Y-m-d'), 
                                                'created'           => date('Y-m-d H:i:s'), 
                                             );
                    $this->assets_product_inout_m->insert_assets_product_inout($array1);

                $asset_category = $this->asset_category_m->get_single_asset_category(array('asset_categoryID' => $single_asset->asset_categoryID));

                $ledger_array[] = [
                                "maininvoiceID" => $assignmentID,
                                "studentID"     => $assignmentID,
                                "entry_userID"  => $assignmentID,
                                "entry_type"    => 'asset',
                                "classesID"     => $assignmentID,
                                "net_fee"       => $this->input->post("assigned_quantity")* $single_asset->average_price,
                                "refrence_no"   => $single_asset->assetID,
                                'description'   => $single_asset->description.' Assignment for asset ' ,
                                "accounts_reg"  => $single_asset->assetID, 
                                "account_credit"=> $asset_category->credit_id, 
                                "account_debit" => $asset_category->debit_id, 
                                "feetypeID"     => 900000901,
                                "date"          => date('Y-m-d'), 
                                "discount"      => 0, 
                                ];            
                ...
                $ledger_array[] = [
                                ...
                                "net_fee"       => $this->input->post("assigned_quantity")* $single_asset->average_price,
                                "refrence_no"   => $single_asset->assetID,
                                'description'   => $single_asset->description.' return for asset ' ,
                                "accounts_reg"  => $single_asset->assetID, 
                                "account_credit"=> $asset_category->debit_id, 
                                "account_debit" => $asset_category->credit_id, 
                                "feetypeID"     => 900000902,
                                "date"          => date('Y-m-d'), 
                                "discount"      => 0, 
                                ];            
        ...
        if($this->input->post('usertypeID') == 0) {
            ...
            if($usertypeID == 1) {
                ...
                if(customCompute($systemadmins)) {
                    echo "<option value=''>".$this->lang->line('asset_assignment_select_user')."</option>";
                ...
                if(customCompute($classes)) {
                    echo "<option value=''>".$this->lang->line('asset_assignment_select_class')."</option>";
