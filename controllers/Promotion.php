...
class Promotion extends Admin_Controller
{
    ...
    protected function rules()
    {
        $rules = [
            ...
            [
                ...
                'field' => 'psectionID',
                'label' => $this->lang->line('promotion_promotion') . ' Section',
                'rules' => 'trim|required|xss_clean|max_length[11]',
            ], [
                'field' => 'jclassesID',
                'label' => $this->lang->line('promotion_promotion') . ' Degree',
                'rules' => 'trim|required|xss_clean|max_length[11]',
            ],
        ];
        return $rules;
    ...
    public function index()
    {
        ...
        if ((int) $classesID && (int) $sectionID) {
            ...
            $this->data['class_sec']    = $this->section_m->general_get_order_by_section(['classesID' => $classesID]);
            ...
            if ($_POST) {
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    $promotionLog = [
                        ...
                        'jumpSchoolYearID'             => $this->session->userdata('defaultschoolyearID'),
                        'sectionID'                    => $sectionID,
                        'jumpSectionID'                => $this->input->post('psectionID'),
                        'status'                       => 0,
                    ...
                    redirect("promotion/add/$classesID/$sectionID");
                }
            } else {
                $this->data["subview"] = "promotion/index";
                $this->load->view('_layout_main', $this->data);
    ...
    public function add()
    {
        ...
        if ((int) $classID && (int) $schoolyearID) {
            ...
            $schoolyear = $this->section_m->get_section($schoolyearID);
            if (customCompute($classes) && customCompute($schoolyear)) {
                ...
                $this->data['currentSchoolYear'] = $this->data['sections'][$schoolyearID];

                $this->data['promotionClass']      = $this->data['classes'][$this->data['promotionClassID']];
                $this->data['promotionSchoolYear'] = $this->data['sections'][$this->data['promotionsectionID']];
    ...
    private function studentPromotionCalculation($classID, $schoolyearID)
    {
        ...
        $this->data['promotionsectionID']      = $promotionLog->jumpSectionID;
    ...
    public function promotion_to_next_class()
    {
        ...
        $sectionID   = $promotionLog->sectionID;
        ...
        if (customCompute($students) && customCompute($studentIDs) && customCompute($previousClasseID)) {
            ...
            foreach ($explodeStudents as $key => $studentID) {
                ...
                if (isset($students[$studentID])) {
                    ...
                    $array = [
                        'classesID'    => $promotionClassID,
                        'schoolyearID' => $promotionYearID,
                       // 'roll'         => isset($enroll) && $enroll ? 0 : $roll,
                        'sectionID'    => $promotionsectionID,
                    ];
                   // var_dump($array);
                    ...
                    if (!customCompute($studentReletion)) {
                        ...
                        $arrayStudentRelation = array(
                                    'srclassesID' => $promotionClassID,
                                    'srsectionID' => $promotionsectionID, 
                                );
                        $this->studentrelation_m->update_studentrelation_with_multicondition($arrayStudentRelation, array('srstudentID' => $studentID, 'srschoolyearID' => $promotionLog->jumpSchoolYearID));
                    }

                    $this->student_m->update_student($array, $studentID);
                    $this->maininvoice_m->update_student_maininvoice(['maininvoice_status' => 0],array('maininvoicestatus' => 2,'maininvoicestudentID' => $studentID));

                    $this->invoice_m->update_student_invoice(['invoice_status' => 0],array('paidstatus' => 2,'studentID' => $studentID));
    ...
    public function promotion_list()
    {
        ...
        if ((int) $classID) {
            $string = base_url("promotion/index/$classID/$sectionID");
            echo $string;
        } else {
            redirect(base_url("promotion/index"));
        }
